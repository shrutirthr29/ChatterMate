  ...
  return (
    ...
    <div className="min-h-full flex flex-col justify-center mt-3 pr-2 w-full">
      <div className="flex justify-between">
        <div>
          <span className="text-greenishblue">{data?.name}</span>
        </div>
        {
          !isContactPage && (
            <div>
              <span className={`${!data.totalUnreadMessages>0 ? "text-secondary" : "text-greenishblue"} text-sm`}>
        ...
        <div className="flex justify-between w-full">
          <span className="text-secondary line-clamp-1 text-sm">
            ...
            }
            {
              ...
              <FaMicrophone className="text-greenishblue" /> Audio
              </span>)
            }
            {
              data.type === "image" && (<span className="flex gap-1 items-center">
              <FaCamera className="text-greenishblue" /> Image
              </span>)
            }
            </div>
            }
          ...
          {
            data.totalUnreadMessages>0 && <span className="bg-greenishblue px-[5px] rounded-full text-sm text-white">{data.totalUnreadMessages}</span>
          }
        </div>
      </div>
    </div>
